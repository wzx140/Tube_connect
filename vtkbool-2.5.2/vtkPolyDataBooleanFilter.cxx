...
int vtkPolyDataBooleanFilter::ProcessRequest(vtkInformation *request, vtkInformationVector **inputVector,
...
void vtkPolyDataBooleanFilter::GetPolyStrips(vtkPolyData *pd, vtkIntArray *conts, vtkIntArray *sources,
                                             PolyStripsType &polyStrips) {
#ifdef DEBUG
...
void vtkPolyDataBooleanFilter::CutCells(vtkPolyData *pd, PolyStripsType &polyStrips) {
    ...
    for (itr = polyStrips.begin(); itr != polyStrips.end(); ++itr) {
        ...
        strips.erase(std::remove_if(strips.begin(), strips.end(), [&](const StripType &s) {
            return pts[s.front().ind].capt == CAPT_NOT && pts[s.back().ind].capt == CAPT_NOT;
        }), strips.end());
...
public:
    PolyAtEdge(vtkPolyData *_pd, int _polyId, int _ptIdA, int _ptIdB) : pd(_pd), polyId(_polyId), ptIdA(_ptIdA),
                                                                        ptIdB(_ptIdB), loc(LOC_NONE) {
        vtkIdList *poly = vtkIdList::New();
