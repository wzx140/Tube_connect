...
void MainWindow::import() {
    ...
    if (!fileName.isEmpty() && (fileName.endsWith(".stl", Qt::CaseInsensitive))) {
        ...
        ui->tableWidget->item(2, 0)->setText(QString::number(10));
...
void MainWindow::run() {
    ...
    auto coefficientStr = ui->tableWidget->item(2, 0)->text();

    double length = lengthStr.toDouble();
    double coefficient = coefficientStr.toDouble();
...
//    time of run
    ...
    graph->setCoefficient(coefficient);
    graph->create(tubes);
    graph->update();
